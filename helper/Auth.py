...
def get_auth_code(client_id: str, redirect_port: int) -> str:
    login_url = f'https://login.live.com/oauth20_authorize.srf?client_id={client_id}&response_type=code&redirect_uri=http://localhost:{redirect_port}&scope=XboxLive.signin%20offline_access'
...
def authenticate(client_id: str, client_secret: str, redirect_port: int, auth_code: str) -> AuthenticationToken:
    login_data = complete_login(client_id, client_secret, f'http://localhost:{redirect_port}', auth_code)
